...
void draw_sprites(t_params *p)
{
    ...
    s->sprite_screenX = ((WINX / 2) * (1 + s->transformX / s->transformY));
    s->sprite_height = fabs(WINY / s->transformY);
    s->draw_startY = -s->sprite_height / 2 + WINY / 2;
    if(s->draw_startY < 0)
        s->draw_startY = 0;
    s->draw_endY = s->sprite_height / 2 + WINY / 2;
    if(s->draw_endY >= WINY)
        s->draw_endY = WINY - 1;
     
    s->sprite_width = fabs(WINY / s->transformY);
